...
#|   on top, which lists functions beginning with str_c. The function signature
...
This is a way of writing non-English characters that work on all systems. You can see the complete list of other special characters in `?Quotes`.
...
When you use the debug mode, you get three extra columns added to the output: `a_ok`, `a_pieces`, and `a_remainder` (if you separate a variable with a different name, you'll get a different prefix).
Here, `a_ok` lets you quickly find the inputs that failed:

```{r}
debug |> filter(!a_ok)
```

`a_pieces` tells us how many pieces were found, compared to the expected 3 (the length of `names`).
`a_remainder` isn't useful when there are too few pieces, but we'll see it again shortly.
